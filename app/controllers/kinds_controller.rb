class KindsController < ApplicationController
  ...
  # GET /kinds.json
  def index
     @kinds = Kind.all.order(:description).page(params[:page]).per(5)
  end
  ...
  def destroy
    ...
    #verifica antes se existe algum contato associado a esse tipo de contato
    quantidade = Contact.where(kind_id: (params[:id])).count;
    
    if quantidade > 0
      respond_to do |format|
        format.html { redirect_to kinds_path, notice: 'Registro não pode ser apagado'}
